...
export async function createMerchantRestaurant(params: {
  name_fr: string;
  cuisine_type?: string;
  canton: string;
  city: string;
  ...
  try {
    ...
    const admin = createAdminClient();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data: existing } = await (admin.from("restaurants") as any)
      .select("id")
      .eq("merchant_id", merchantId)
      .limit(1)
      .single();

    if (existing) return { success: false, error: "Vous avez déjà un restaurant" };
    ...
    const { error } = await (admin.from("restaurants") as any).insert({
      ...
      name_de: params.name_fr,
      name_en: params.name_fr,
      slug,
