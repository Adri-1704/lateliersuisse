...
export default async function MerchantDashboardPage({
  ...
  return (
    <div className="space-y-6">
      ...
      {!restaurant && (
        <Card className="border-[var(--color-just-tag)]/30 bg-[var(--color-just-tag)]/5">
          <CardContent className="flex items-center gap-4 pt-6">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[var(--color-just-tag)] text-white">
              <UtensilsCrossed className="h-6 w-6" />
            </div>
            <div className="flex-1">
              <h3 className="font-semibold text-lg">Creez votre restaurant</h3>
              <p className="text-sm text-muted-foreground">
                Commencez par creer votre fiche restaurant pour apparaitre sur Just-Tag.
              </p>
            </div>
            <Button asChild className="bg-[var(--color-just-tag)] hover:bg-[var(--color-just-tag)]/90">
              <Link href={`/${locale}/espace-client/mon-restaurant`}>
                Creer <ArrowRight className="ml-2 h-4 w-4" />
              </Link>
            </Button>
          </CardContent>
        </Card>
